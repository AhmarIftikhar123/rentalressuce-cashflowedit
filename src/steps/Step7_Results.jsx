 ...
 * Refined to match Screenshot 3 precisely.
 */

const CardTitle = ({ title, icon, color = 'text-audit-neutral' }) => (
  <h3 className={`text-[12px] font-black uppercase tracking-widest ${color} flex items-center gap-2 mb-6`}>
    <span className={`dashicons ${icon} !text-[20px] !w-5 !h-5 !flex !items-center !justify-center !leading-none`}></span>
    {title}
  </h3>
);

const MtrTag = () => <span className="ml-2 inline-block px-1.5 py-0.5 bg-[#E1EFFE] text-[#1E429F] text-[8px] font-black uppercase tracking-wider rounded">Mid-term</span>;
const StrTag = () => <span className="ml-2 inline-block px-1.5 py-0.5 bg-[#FEF3C7] text-[#92400E] text-[8px] font-black uppercase tracking-wider rounded">Short-term</span>;
...
const ValueBox = ({ label, value, tag = null, isRed = false }) => (
  <div className="bg-white rounded-[16px] px-5 py-4 flex justify-between items-center shadow-sm border border-step-border/40 mb-3">
    ...
    <span className={`text-[17px] font-black ${isRed ? 'text-audit-negative' : 'text-brand-black'}`}>{value}</span>
  </div>
);
    ...
    <div className="bg-[#F7F5F0]">
        ...
        <div className="bg-gradient-to-br from-[#4D545E] to-[#2D3540] rounded-[24px] px-10 py-12 md:py-16 shadow-2xl relative overflow-hidden text-center md:text-left">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-4xl md:text-6xl font-black text-white leading-tight mb-5 max-w-2xl">
              <span className="text-brand-gold">+{fmtMonthly(r.upside)}</span> potential monthly income you may be leaving on the table
            </h2>
            <p className="text-sm md:text-base text-white/50 max-w-xl leading-relaxed font-medium">
              Based on current market volatility and your local equity position, your portfolio is under-performing its maximum yield capacity.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white/40 p-8 rounded-[32px] border border-white/60 shadow-sm">
          ...
          <ValueBox label="Market Rent" value={fmtMonthly(r.baseLTR)} />
          <ValueBox label="PITI" value={fmtMonthly(r.piti)} isRed={true} />
          ...
          <div className="bg-[#F9F8F1] rounded-[24px] p-6 border border-[#E9E5DB] mt-8 flex flex-col items-center text-center">
            <div className="w-12 h-12 rounded-full bg-brand-gold/10 text-brand-gold flex items-center justify-center mb-3">
               <span className="dashicons dashicons-money-alt !text-[24px] !w-6 !h-6 !flex !items-center !justify-center !leading-none"></span>
            </div>
            <span className="text-[10px] font-black tracking-widest uppercase text-brand-dark opacity-60 mb-1">Monthly Net</span>
            <span className="text-4xl font-black text-brand-gold tracking-tight">{fmtMonthly(r.ltr_cf)}</span>
          </div>
        </div>

        {/* Right Column: Optimized Potential */}
        <div className="bg-white/40 p-8 rounded-[32px] border border-white/60 shadow-sm">
          <CardTitle title="Optimized Potential" icon="dashicons-performance" color="text-brand-gold" />
          
          <ValueBox label="MTR Income" value={fmtMonthly(r.baseMTR)} tag={<MtrTag />} />
          <ValueBox label="STR Income" value={fmtMonthly(r.baseSTR)} tag={<StrTag />} />
          
          {/* Optimized Flow Block */}
          <div className="bg-gradient-to-r from-[#8B6B23] to-[#D5A737] rounded-[24px] p-6 border-none mt-8 flex flex-col items-center text-center shadow-xl shadow-brand-gold/20">
            <div className="w-12 h-12 rounded-full bg-white/20 text-white flex items-center justify-center mb-3">
               <span className="text-[24px] leading-none">⚡</span>
            </div>
            <span className="text-[10px] font-black tracking-widest uppercase text-white/70 mb-1">Optimized Cash Flow</span>
            <span className="text-4xl font-black text-white tracking-tight">{fmtMonthly(r.str_cf)}</span>
          </div>
        </div>
      </div>

      {/* 3. EQUITY FOOTER */}
      <div className="bg-white px-8 py-10 md:px-16 md:py-14 border-t border-step-border/50 relative overflow-hidden">
            ...
            <div className="w-full mb-10">
                <div className="flex justify-between items-end mb-4">
                    <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-lg bg-brand-gold/5 text-brand-gold flex items-center justify-center">
                             <span className="dashicons dashicons-admin-multisite !text-[18px]"></span>
                        </div>
                        <span className="text-[11px] font-black text-audit-neutral uppercase tracking-widest">Equity Position</span>
                    </div>
                    <div className="text-right">
                        <span className="block text-[10px] font-bold text-audit-neutral uppercase tracking-widest mb-1">Estimated Equity</span>
                        <span className="text-3xl md:text-5xl font-black text-brand-gold tracking-tight">{fmtMoney(r.equity)}</span>
                    </div>
                </div>
                ...
                <div className="h-4 bg-[#F2EFE8] rounded-full overflow-hidden w-full mb-3 shadow-inner">
                    <div 
                        ...
                        style={{ width: `${Math.max(2, Math.min(100, Math.abs(r.equityPct)))}%` }}
                    />
                </div>
